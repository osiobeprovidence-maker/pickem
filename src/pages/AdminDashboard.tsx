    ...
    <div className="min-h-screen bg-stone-50 px-4 py-10 md:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-8">
        <motion.section
          ...
          className="overflow-hidden rounded-[2.75rem] border border-apple-gray-100 bg-white shadow-sm"
        >
          <div className="bg-apple-gray-500 px-6 py-8 text-white md:px-10 md:py-10">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="space-y-4">
                <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-[10px] font-black uppercase tracking-[0.35em]">
                  <Crown className="h-3.5 w-3.5" />
                  {isSuperAdmin ? 'Super Admin' : 'Admin Center'}
                </div>
                <div>
                  <h1 className="text-4xl font-black tracking-tighter md:text-6xl">Admin Dashboard</h1>
                  <p className="mt-3 max-w-2xl text-sm font-bold text-apple-gray-100/90 md:text-base">
                    Review platform access, approve admins, and manage business and runner verification from one control room.
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                {isSuperAdmin && (
                  <button
                    type="button"
                    onClick={() => setIsInviteModalOpen(true)}
                    className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-black text-apple-gray-500 shadow-sm"
                  >
                    <UserPlus className="h-4 w-4" />
                    Add Admin
                  </button>
                )}
                <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-5 py-3 text-sm font-black text-white ring-1 ring-white/20">
                  <BadgeCheck className="h-4 w-4" />
                  {user.verification_cleared ? 'Fully Verified' : 'Verification In Progress'}
                </div>
              </div>
            </div>
          </div>

          <div className="flex flex-wrap gap-3 border-t border-apple-gray-100 px-6 py-5 md:px-8">
            {tabButtons.map(tab => (
              <button
                key={tab.id}
                type="button"
                onClick={() => openTab(tab.id)}
                className={cn(
                  'rounded-full px-5 py-3 text-sm font-black transition-colors',
                  activeTab === tab.id
                    ? 'bg-apple-gray-500 text-white'
                    : 'bg-apple-gray-50 text-apple-gray-400 hover:text-apple-gray-500',
                )}
              >
                {tab.label}
              </button>
        ...
        {activeTab === 'overview' && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-[2rem] border border-apple-gray-100 bg-white p-6 shadow-sm">
                <div className="text-[10px] font-black uppercase tracking-[0.3em] text-apple-gray-200">Approved Admins</div>
                <div className="mt-4 text-4xl font-black tracking-tight text-apple-gray-500">{approvedAdmins.length}</div>
              </div>
              <div className="rounded-[2rem] border border-apple-gray-100 bg-white p-6 shadow-sm">
                <div className="text-[10px] font-black uppercase tracking-[0.3em] text-apple-gray-200">Pending Admins</div>
                <div className="mt-4 text-4xl font-black tracking-tight text-apple-gray-500">
                  {admins.filter(admin => admin.status === 'pending').length}
                </div>
              </div>
              <div className="rounded-[2rem] border border-apple-gray-100 bg-white p-6 shadow-sm">
                <div className="text-[10px] font-black uppercase tracking-[0.3em] text-apple-gray-200">Business Reviews</div>
                <div className="mt-4 text-4xl font-black tracking-tight text-apple-gray-500">{pendingBusinesses.length}</div>
              </div>
              <div className="rounded-[2rem] border border-apple-gray-100 bg-white p-6 shadow-sm">
                <div className="text-[10px] font-black uppercase tracking-[0.3em] text-apple-gray-200">Runner Reviews</div>
                <div className="mt-4 text-4xl font-black tracking-tight text-apple-gray-500">{pendingRunners.length}</div>
              </div>
            </div>
            ...
            <div className="grid gap-6 xl:grid-cols-2">
              <div className="rounded-[2.25rem] border border-apple-gray-100 bg-white p-8 shadow-sm">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <h2 className="text-2xl font-black tracking-tight text-apple-gray-500">Admin Access</h2>
                    <p className="mt-2 text-sm font-bold text-apple-gray-300">Manage who gets into the admin side of Pick&apos;em.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => openTab('admins')}
                    className="inline-flex items-center gap-2 rounded-full bg-apple-gray-50 px-5 py-3 text-xs font-black uppercase tracking-widest text-apple-gray-500"
                  >
                    Open
                    <ArrowRight className="h-3.5 w-3.5" />
                  </button>
              ...
              <div className="rounded-[2.25rem] border border-apple-gray-100 bg-white p-8 shadow-sm">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <h2 className="text-2xl font-black tracking-tight text-apple-gray-500">Verification Queue</h2>
                    <p className="mt-2 text-sm font-bold text-apple-gray-300">Review runner and business verification that still needs approval.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => openTab(pendingBusinesses.length ? 'businesses' : 'runners')}
                    className="inline-flex items-center gap-2 rounded-full bg-apple-gray-50 px-5 py-3 text-xs font-black uppercase tracking-widest text-apple-gray-500"
                  >
                    Open
                    <ArrowRight className="h-3.5 w-3.5" />
        ...
        {activeTab === 'admins' && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            {admins.map(admin => (
              <div key={admin.id} className="rounded-[2rem] border border-apple-gray-100 bg-white p-6 shadow-sm">
                <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                  <div className="space-y-3">
                    <div className="flex flex-wrap items-center gap-3">
                      <div className="text-2xl font-black tracking-tight text-apple-gray-500">{admin.name}</div>
                      <span
                        className={cn(
                    ...
                    <div className="text-sm font-bold text-apple-gray-300">
                      {admin.hasPassword ? 'Password set' : 'Password not set yet'}
                    </div>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      ...
                      {admin.status !== 'removed' && (
                        <button
                          ...
                          className="rounded-full border border-red-100 bg-red-50 px-5 py-3 text-sm font-black text-red-600"
                        >
                          Remove Admin
                        </button>
                      )}
        ...
        {activeTab === 'businesses' && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            {pendingBusinesses.length === 0 ? (
              <div className="rounded-[2rem] border border-apple-gray-100 bg-white p-10 text-center shadow-sm">
                <Building2 className="mx-auto h-10 w-10 text-apple-gray-200" />
                <div className="mt-4 text-2xl font-black text-apple-gray-500">No pending business reviews</div>
              </div>
            ) : (
              pendingBusinesses.map(business => (
                <div key={business.id} className="rounded-[2rem] border border-apple-gray-100 bg-white p-6 shadow-sm">
                  <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                    <div className="space-y-3">
                      <div className="text-2xl font-black tracking-tight text-apple-gray-500">{business.name || 'Unnamed business'}</div>
                      <div className="text-sm font-bold text-apple-gray-300">{business.category || 'No category yet'}</div>
                      <div className="text-sm font-bold text-apple-gray-300">{business.address || 'No address yet'}</div>
                    </div>
                    <button
                      type="button"
        ...
        {activeTab === 'runners' && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
            {pendingRunners.length === 0 ? (
              <div className="rounded-[2rem] border border-apple-gray-100 bg-white p-10 text-center shadow-sm">
                <Truck className="mx-auto h-10 w-10 text-apple-gray-200" />
                <div className="mt-4 text-2xl font-black text-apple-gray-500">No pending runner reviews</div>
              </div>
            ) : (
              pendingRunners.map(record => (
                <div key={record.runner_id} className="rounded-[2rem] border border-apple-gray-100 bg-white p-6 shadow-sm">
                  <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                    <div className="space-y-3">
                      <div className="text-2xl font-black tracking-tight text-apple-gray-500">{record.full_name || 'Unnamed runner'}</div>
                      <div className="text-sm font-bold text-apple-gray-300">{record.email}</div>
                      <div className="text-sm font-bold text-apple-gray-300">{record.residential_address || 'No address submitted yet'}</div>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleApproveRunner(record.runner_id)}
                      className="rounded-full bg-emerald-600 px-5 py-3 text-sm font-black text-white"
                    >
                      Approve Runner
                    </button>
      ...
      {isInviteModalOpen && (
        <div className="fixed inset-0 z-[210] flex items-center justify-center bg-black/55 px-4 backdrop-blur-sm">
          <motion.div
            initial={{ opacity: 0, scale: 0.96, y: 8 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="w-full max-w-xl rounded-[2.5rem] bg-white p-8 shadow-2xl"
          >
            <div className="flex items-start justify-between gap-6">
              <div className="space-y-2">
                <div className="inline-flex h-14 w-14 items-center justify-center rounded-[1.25rem] bg-apple-gray-50 text-apple-gray-500">
                  <UserPlus className="h-7 w-7" />
                </div>
                <div className="text-3xl font-black tracking-tight text-apple-gray-500">Add Admin</div>
                <p className="text-sm font-bold text-apple-gray-300">
                  Add a new admin by email. They can log in like a normal user, and once approved they&apos;ll get the admin button too.
                </p>
              </div>
              <button
                type="button"
                onClick={() => setIsInviteModalOpen(false)}
                className="rounded-full bg-apple-gray-50 p-3 text-apple-gray-300"
              >
                <X className="h-5 w-5" />
              </button>
            </div>

            <form onSubmit={handleInviteAdmin} className="mt-8 space-y-5">
              <div className="space-y-3">
                <label className="ml-4 text-[10px] font-black uppercase tracking-[0.3em] text-apple-gray-200">
                  Admin Email
                </label>
                <input
                  type="email"
                  ...
                  placeholder="admin@pickem.com"
                  className="w-full rounded-full border-none bg-apple-gray-50 px-8 py-5 font-bold text-apple-gray-500 shadow-sm outline-none"
                />
              </div>

              <div className="rounded-[1.75rem] bg-apple-gray-50 px-6 py-5 text-sm font-bold text-apple-gray-300">
                New admins start in pending mode until the super admin approves them.
              </div>
              ...
              <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                <button className="rounded-full bg-apple-gray-500 px-6 py-4 text-sm font-black text-white">
                  Save Admin
                </button>
              </div>
            </form>
