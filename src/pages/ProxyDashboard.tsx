...
import { ShieldCheck, Clock, CheckCircle2, MapPin, Plus, Search, Key } from 'lucide-react';
  ...
  useEffect(() => {
    if (user) {
      api.getDeliveries('customer', user.id).then(all => {
        setDeliveries(all.filter(d => d.type === 'proxy'));
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-apple-gray-300 font-medium">Manage your secure pickup codes and delegated deliveries.</p>
        </div>
        <Link
          to="/proxy-pickup"
        ...
        >
          <Plus className="w-5 h-5" /> New Proxy
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-8 rounded-[2rem] border border-apple-gray-100 shadow-sm">
          ...
          <div className="text-sm text-apple-gray-300 font-bold uppercase tracking-wider">Active Proxies</div>
        </div>
        <div className="bg-white p-8 rounded-[2rem] border border-apple-gray-100 shadow-sm">
      ...
      <div className="bg-white rounded-[2.5rem] border border-apple-gray-100 shadow-sm overflow-hidden">
        ...
        {isLoading ? (
          <div className="p-20 text-center text-apple-gray-200 font-medium">Loading your proxies...</div>
        ) : deliveries.length === 0 ? (
          <div className="p-20 text-center">
            <div className="w-20 h-20 bg-apple-gray-50 rounded-full flex items-center justify-center mx-auto mb-6">
              <ShieldCheck className="w-10 h-10 text-apple-gray-200" />
            </div>
            <h3 className="text-2xl font-bold text-apple-gray-500 mb-2">No proxy pickups yet</h3>
            <p className="text-apple-gray-300 mb-8 font-medium">Need someone to receive a package for you? Generate a proxy code.</p>
            <Link
              to="/proxy-pickup"
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex items-start gap-6">
                    <div className="w-14 h-14 rounded-2xl bg-apple-gray-50 flex items-center justify-center shrink-0">
                      <ShieldCheck className="w-7 h-7 text-apple-gray-500" />
                    </div>
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        <span className={cn(
                          "px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider",
                          delivery.status === 'delivered' ? "bg-emerald-100 text-emerald-700" : "bg-indigo-100 text-indigo-700"
                        )}>
                          {delivery.status}
                        </span>
                      ...
                      <div className="flex flex-col gap-2 text-[14px] text-apple-gray-300 font-medium">
                        <div className="flex items-center gap-2">
                          <MapPin className="w-4 h-4" /> Pickup: {delivery.pickup_location}
                        </div>
                        <div className="flex items-center gap-2">
                          <MapPin className="w-4 h-4" /> Destination: {delivery.drop_location}
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-4">
                    {delivery.proxy_code && (
                      <div className="bg-apple-gray-500 text-white px-6 py-2 rounded-xl text-lg font-mono font-bold shadow-sm">
                        {delivery.proxy_code}
                      </div>
                    )}
                    <div className="text-xl font-bold text-apple-gray-500">₦{delivery.fee}</div>
                  </div>
                </div>
              </div>
            ))}
