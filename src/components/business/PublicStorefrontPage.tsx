...
import type { Business, Product, Storefront } from '../../types';
...
export function PublicStorefrontPage({ business, storefront, products, publicUrl, isPreview }: PublicStorefrontPageProps) {
      ...
      <div className="relative">
        <div className="h-[30vh] min-h-[240px] bg-apple-gray-500 sm:h-[38vh]">
          {storefront.banner_url ? (
            <img src={storefront.banner_url} alt="" className="h-full w-full object-cover" />
          ) : (
              ...
              <div className="p-6 sm:p-8">
                {isPreview ? (
                  <div className="mb-5 rounded-2xl border border-brand-100 bg-brand-50 px-4 py-3 text-sm font-medium text-brand-800">
                    Preview mode is on. Only you can see this unpublished storefront.
                  </div>
                ) : null}
                ...
                <div className="mt-8 grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
                  <div className="rounded-[1.75rem] border border-apple-gray-100 bg-apple-gray-50 p-5">
                    <div className="text-[11px] font-black uppercase tracking-[0.16em] text-apple-gray-300">About this store</div>
                    <p className="mt-3 text-sm font-medium leading-relaxed text-apple-gray-400 sm:text-base">
                      {storefront.description || business.description}
                    </p>
                  </div>
                  <div className="rounded-[1.75rem] border border-apple-gray-100 bg-white p-5">
                    <div className="text-[11px] font-black uppercase tracking-[0.16em] text-apple-gray-300">Storefront URL</div>
                    <div className="mt-3 break-all text-sm font-bold text-apple-gray-500">{publicUrl}</div>
                  </div>
                </div>

                <div className="mt-4 grid gap-4 lg:grid-cols-3">
                  <div className="rounded-2xl bg-apple-gray-50 p-4">
                    <div className="text-xs font-black uppercase tracking-[0.16em] text-apple-gray-300">Category</div>
                    <div className="mt-2 text-sm font-bold text-apple-gray-500">{storefront.category || business.category}</div>
                  </div>
                  <div className="rounded-2xl bg-apple-gray-50 p-4">
                    <div className="text-xs font-black uppercase tracking-[0.16em] text-apple-gray-300">Contact email</div>
                    <div className="mt-2 text-sm font-bold text-apple-gray-500">{storefront.contact_email || business.email}</div>
                  </div>
                  <div className="rounded-2xl bg-apple-gray-50 p-4">
                    <div className="text-xs font-black uppercase tracking-[0.16em] text-apple-gray-300">Contact phone</div>
                    <div className="mt-2 text-sm font-bold text-apple-gray-500">{storefront.contact_phone || business.phone}</div>
                  </div>
                </div>
              </div>
            </Card>
        ...
        <Card className="p-6 sm:p-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black text-apple-gray-500">Store listings</h2>
              <p className="mt-2 text-sm font-medium leading-relaxed text-apple-gray-300">
                Browse available listings from this business storefront.
              </p>
            </div>
                      ...
                      <div className="mt-5 flex items-end justify-between gap-4">
                        <div>
                          ...
                          {product.discount_price ? (
                            <div className="mt-1 text-sm font-medium text-apple-gray-300">Discount ₦{product.discount_price.toLocaleString()}</div>
                          ) : null}
                        </div>
